...
def adam(NN,data_set,Flattened_Param,batch_size,sequence_length,input_size, max_it=1000, step_size=0.01, b1=0.9, b2=0.999, eps=10**-8,C=0):
    ...
    for t in range(max_it):
        ...
        images,labels = Utils.next_mini_batch(data_set,batch_size,sequence_length,input_size)
...
def sca_ridge(NN,Flattened_Param,data_set,batch_size,sequence_length,input_size,step_size=0.12,step_size_eps=0.0
    ...
    for t in range(max_it):
        ...
        for p in range(virtual_processors):
            ...
            # Update variable
            Flattened_Param[idx] = (1-step_size)*Flattened_Param[idx] + torch.from_numpy(step_size*par_hat).float()
